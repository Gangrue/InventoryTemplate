...
function inventory_draw(){
	...
	for(var i=0; i<cur_inventory_max_slot_length; i++) {
		...
		var cur_slot = ii + row_slot_count;
		var inventory_slot_x = inventory_x+slot_offset+((i%row_slot_count)*slot_width);
		var inventory_slot_y = inventory_y+slot_offset+((floor(i/row_slot_count))*(slot_width+slot_offset));
	...
	inventory_draw_hovering();
	inventory_draw_selecting();
}